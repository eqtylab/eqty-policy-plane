...
import { PipelineState, NodeStatus, PipelineOutput } from './types';
...
export type PipelineAction =
    ...
    | { type: 'ADD_LOG'; payload: { nodeId: string; log: { message: string; timestamp: number; level: 'info' | 'warning' | 'error' } } }
    | { type: 'COMPLETE_NODE'; payload: { nodeId: string; outputs: PipelineOutput[] } }
    | { type: 'OVERRIDE_GUARDRAIL'; payload: { alertId: string; reason: string; timestamp: number } };

const logStateChange = (action: PipelineAction, prevState: PipelineState, nextState: PipelineState) => {
    ...
    switch (action.type) {
        ...
        case 'ADD_LOG':
            console.log('New Log Entry:', {
                nodeId: action.payload.nodeId,
                message: action.payload.log.message,
                level: action.payload.log.level,
                timestamp: new Date(action.payload.log.timestamp)
            });
            break;
    }
    ...
    console.log('State Changes:', {
        pipelineStatus: prevState.status !== nextState.status ? {
            from: prevState.status,
            to: nextState.status
        } : 'unchanged',
        completedNodes: {
        ...
        case 'ADD_LOG':
            nextState = {
                ...
                nodes: {
                    ...
                    [action.payload.nodeId]: {
                        ...state.nodes[action.payload.nodeId],
                        logs: [...state.nodes[action.payload.nodeId].logs, action.payload.log]
                    }
                }
            };
            break;
