...
function ConsoleDrawerRight({
  ...
  return (
    <div
      className={`tw-fixed tw-top-0 tw-bottom-0 tw-right-0 tw-z-50 tw-flex tw-transition-transform tw-duration-300 tw-ease-in-out
                     ${
                       isOpen
                         ? "tw-translate-x-0"
    ...
    >
      ...
      <div
        onClick={onToggle}
        className="tw-rounded-tl-xl  tw-rounded-bl-xl   tw-flex tw-items-center tw-justify-center tw-w-8 tw-bg-branddialogbg/40 hover:tw-bg-branddialogbg/80  tw-cursor-pointer"
      >
        <div className="tw-w-4 tw-h-4 tw-text-brandblue">
          <Terminal />
        </div>
      ...
      {/* Main Drawer */}
      <div
        className={`tw-flex tw-flex-col tw-w-[800px] tw-h-full 
                     
                     tw-border-l !tw-border-white/10 tw-bg-black`}
      >
        {/* Table Header */}
        <div className="tw-bg-[#2d293b] tw-border-b !tw-border-white/10">
          <div className="tw-grid tw-grid-cols-[100px_100px_1fr_1fr] tw-gap-2 tw-px-4 tw-py-2">
            <div className="tw-text-xs tw-font-medium tw-text-white/60">
              Time
            </div>
            <div className="tw-text-xs tw-font-medium tw-text-white/60">
              Type
        ...
        {/* Logs Content */}
        <div ref={scrollRef} className="tw-flex-1 tw-overflow-y-auto">
          {logEntries.map((entry, index) => {
            const Icon = iconMap[entry.type];
            return (
              <div
                key={entry.id}
                className={`tw-grid tw-grid-cols-[100px_100px_1fr_1fr] tw-gap-2 
                             tw-px-4 tw-py-2 tw-border-b !tw-border-white/5
                             ${rowHighlightMap[entry.type]}
                             ${index !== 0 ? "tw-animate-fade-in" : ""}`}
              >
                <div className="tw-text-xs tw-text-white/80">
                  {entry.timestamp}
                </div>
                <div className="tw-flex tw-items-center tw-gap-1.5">
                  <div className={`tw-w-3.5 tw-h-3.5 ${colorMap[entry.type]}`}>
                    <Icon />
                  </div>
                  <span
                    className={`tw-text-xs tw-font-medium ${
                ...
                <div className="tw-text-xs tw-text-white">{entry.content}</div>
                <div className="tw-text-xs tw-text-white/60">
                  {entry.details}
                </div>
              </div>
            );
